...
conn = ibm_db.connect(
    "DATABASE=;HOSTNAME=;PORT=;SECURITY=SSL;SSLSererCertificate=DigiCertGlobalRootCA.crt;UID=;PWD=",
    "",
    "",
)
...
def add_product():
    ...
    if request.method == "POST" and form.validate():
        ...
        sql1 = (
            "INSERT INTO products(product_id, product_cost, product_num) VALUES(?,?,?)"
        )
        stmt1 = ibm_db.prepare(conn, sql1)
        ...
        if from_location == to_location:
            ...
            if result != False:
                if (len(result)) > 0:
                    ...
                    if alert_num <= 0:
                        alert(
                            "Please Add {} number of {} to {} warehouse!".format(
                                -q, product_id, from_location
                            )
                        )
            else:
                raise CustomError(
                    "There is no product named {} in {}.".format(
            ...
            if f == 1:
                sql = (
                    "SELECT * from product_balance where location_id=? and product_id=?"
                )
                stmt = ibm_db.prepare(conn, sql)
